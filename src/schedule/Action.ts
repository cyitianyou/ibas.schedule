...
export default abstract class Action {
    ...
    addConfig(key: string, value: any): void {
        ...
        this.log(ibas.emMessageLevel.INFO, "new config item [{1} - {2}].",
            ibas.objects.isNull(this.name) ? this.id : this.name, key, value);
    }
    /**
     * 获取配置
     * @param key 配置项
     ...
     * @param pars 格式内容
     */
    protected log(level: ibas.emMessageLevel, message: string, ...pars: any[]): void;
    /**
     * 记录消息
     * @param message 内容
     * @param pars 格式内容
    ...
    protected log(): void {
        ...
        let pars: any[] = [];
        let heard: string = ibas.strings.format("{0}: ", ibas.objects.isNull(this.name) ? ibas.objects.getTypeName(this) : this.name);
    ...
    protected done(): void {
        ...
        this.log(ibas.emMessageLevel.INFO, "action was completed at [{0}], during [{1}]s.",
            this.endTime.toLocaleString(),
            ibas.dates.difference(ibas.dates.emDifferenceType.SECOND, this.endTime, this.startTime));
